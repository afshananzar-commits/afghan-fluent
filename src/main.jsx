...
function awardXP(gs,n,reason,level=null){gs.updateGame(g=>{const d=g.daily?.date===dayKey()?g.daily:{date:dayKey(),xp:0},lp={...(g.levelPoints||{})},before=g.xp||0,after=before+n,bonus=Math.max(0,Math.floor(after/100)-Math.floor(before/100));if(level)lp[level]=(lp[level]||0)+n;return{...g,xp:after,levelPoints:lp,kiteTickets:(g.kiteTickets||0)+bonus,daily:{date:dayKey(),xp:(d.xp||0)+n},lastActivity:{date:new Date().toISOString(),reason,amount:n}}})}
...
function App(){
 ...
 return <div className={`app ${mode==='kids'?'kids-mode':''}`}><DesktopRail tab={tab} go={go} streak={app.progress.streak||0} isAdmin={profile.role==='admin'}/><div className="app-stage"><TopChrome mode={mode} setMode={setModePersist} displayName={displayName}/><main>
  ...
  {tab==='path'&&<LearningPath app={app} game={game} openLesson={l=>{setSelectedLesson(l);go('games')}}/>}
  {tab==='games'&&<Games app={app} game={game} go={go} selectedLesson={selectedLesson} clearSelectedLesson={()=>setSelectedLesson(null)}/>}
...
function ListeningQuiz({game,level,lesson,onSolved}){
 const pool=useMemo(()=>{const base=lesson?.sentences?.length?lesson.sentences:sentences;return base.filter(s=>(s.spoken||s.latin)&&s.dutch)},[lesson?.id]),[idx,setIdx]=useState(game.game.positions?.listen||0),[choice,setChoice]=useState(null),[coachMessage,setCoachMessage]=useState(null),[help,setHelp]=useState({hint:false,revealed:false,wrong:0}),s=pool[idx%Math.max(1,pool.length)];
 ...
 const answer=o=>{if(choice)return;const ok=o.id===s.id;setChoice({id:o.id,ok});practice(game,`listen:${s.id}`,ok);if(ok){const pts=rewardFor(15,help);awardXP(game,pts,'luisterquiz',level);completeMission(game,level,'listen');onSolved?.('listen');setCoachMessage({type:'correct',text:`Goed gehoord! +${pts} vliegerpunten ✨`})}else{setHelp(h=>({...h,wrong:h.wrong+1}));setCoachMessage({type:'almost',text:'Bijna. Probeer opnieuw; na fouten daalt de beloning iets.'})}};
 ...
 return <section className="game-card sentence-builder-v29 unified-game-card listening-unified">
   <div className="builder-v29-head unified-game-head">
     <div className="builder-v29-title"><small>🎧 LUISTERQUIZ <span className="builder-xp-inline">max 15 VP</span></small><h2>Luister en kies<br/>het juiste antwoord</h2><p>Wat hoor je?</p></div>
   ...
   <UnifiedCoachZone message={coachMessage} onReset={reset} onPrimary={choice?(choice.ok?next:reset):play} primaryLabel={choice?(choice.ok?'Volgende':'Nog eens'):'Luisteren'}/>
 </section>
}

function PictureQuiz({game,level,lesson,onSolved}){
 const pool=useMemo(()=>lesson?.words?.length?lesson.words:vocab.filter(w=>Number(w.id)>=1),[lesson?.id]),[idx,setIdx]=useState(game.game.positions?.picture||0),[choice,setChoice]=useState(null),[coachMessage,setCoachMessage]=useState(null),[help,setHelp]=useState({hint:false,revealed:false,wrong:0}),w=pool[idx%Math.max(1,pool.length)];
 ...
 const answer=o=>{if(choice)return;const ok=o.id===w.id;setChoice({id:o.id,ok});practice(game,`picture:${w.id}`,ok);if(ok){const pts=rewardFor(10,help);awardXP(game,pts,'plaatjesquiz',level);completeMission(game,level,'picture');onSolved?.('picture');setCoachMessage({type:'correct',text:`Precies! +${pts} vliegerpunten ✨`})}else{setHelp(h=>({...h,wrong:h.wrong+1}));setCoachMessage({type:'almost',text:'Bijna. Probeer opnieuw; na fouten daalt de beloning iets.'})}};
 const reset=()=>{setChoice(null);setCoachMessage(null)};
 ...
 return <section className="game-card sentence-builder-v29 unified-game-card picture-unified">
   <div className="builder-v29-head unified-game-head">
     <div className="builder-v29-title"><small>🖼️ PLAATJESQUIZ <span className="builder-xp-inline">max 10 VP</span></small><h2>Welke afbeelding<br/>hoort erbij?</h2><p>Kies het plaatje dat past bij het woord.</p></div>
   ...
   <UnifiedCoachZone message={coachMessage} onReset={reset} onPrimary={choice?(choice.ok?next:reset):listen} primaryLabel={choice?(choice.ok?'Volgende':'Nog eens'):'Luisteren'}/>
 </section>
}
...
function SpeedRound({game,level,lesson,onSolved}){
 ...
 useEffect(()=>{if(running&&seconds<=0){setRunning(false);clearTimeout(feedbackTimer.current);const pts=rewardFor(score*5,{hint:assists>0,wrong:wrongs,revealed:assists>2});awardXP(game,pts,'snelle ronde',level);if(score>=3){completeMission(game,level,'speed');onSolved?.('speed')}setCoachMessage({type:'celebrate',text:`Ronde klaar! ${score} goed · +${pts} vliegerpunten 🎉`})}},[seconds]);
...
function Games({app,game,go,selectedLesson,clearSelectedLesson}){
 ...
 const missions=new Set(game.game.levelMissions?.[level]||[]),locked=level>game.level;
 const choose=t=>{if(locked)return;if(t==='kite'){if((game.game.kiteTickets||0)<=0)return;game.updateGame(g=>({...g,kiteTickets:Math.max(0,(g.kiteTickets||0)-1)}))}setType(t);game.updateGame(g=>({...g,lastGame:t}))};
 const backToMenu=()=>setType(null); const solved=()=>{}; const kiteComplete=n=>{if(n>0)awardXP(game,n*5,'vlieger avontuur',null)};
