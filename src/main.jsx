...
function buildCurriculum(){
 ...
 for(let i=0;i<50;i++){
  ...
  levels.push({id:`level-${i+1}`,number:i+1,title:LEVEL_TITLES[i]||`Level ${i+1}`,category,emoji:iconFor(category),words,sentences:levelSentences,difficulty:i<10?'Starter':i<25?'Basis':i<40?'Gesprek':'Vloeiender'});
 }
 return levels;
}
...
function saveMissionResult(gs,level,type,correct,total,minCorrect=0){const accuracy=total?Math.round(correct/total*100):0,passed=total>0&&correct>=minCorrect&&accuracy>80;gs.updateGame(g=>{const levelResult={...(g.levelResults?.[level]||{}),[type]:{correct,total,accuracy,passed,finishedAt:new Date().toISOString()}},levelResults={...(g.levelResults||{}),[level]:levelResult},missions={...(g.levelMissions||{})},done=new Set(missions[level]||[]);if(passed)done.add(type);else done.delete(type);missions[level]=[...done];const allPassed=MISSION_TYPES.every(m=>levelResult[m]?.passed===true),completed=new Set(g.completedLevels||[]);let tickets=g.kiteTickets||0;if(allPassed&&!completed.has(level)){completed.add(level);tickets+=1}return{...g,levelResults,levelMissions:missions,completedLevels:[...completed].sort((a,b)=>a-b),kiteTickets:tickets}});return{accuracy,passed}}
...
function GameSummary({game}){const pct=Math.min(100,Math.round(game.daily/game.goal*100)),results=game.game.levelResults?.[game.level]||{},missions=new Set(MISSION_TYPES.filter(m=>results[m]?.passed===true));return <section className="game-summary"><div className="game-stat"><span><Trophy/></span><div><small>LEVEL</small><b>{game.level}/50</b></div></div><div className="game-xp"><div><b>{game.xp} vliegerpunten</b><small>{missions.size}/4 missies in level {game.level}</small></div><div className="game-bar"><i style={{width:`${missions.size*25}%`}}/></div></div><div className="game-stat"><span><Sparkles/></span><div><small>VLIEGERS</small><b>{game.kiteTickets}</b></div></div><div className="game-goal"><div className="game-bar"><i style={{width:`${pct}%`}}/></div><small>{game.daily}/{game.goal} vandaag</small></div></section>}
...
function Today({app,game,go,displayName='Leerling'}){const known=app.knownIds.size;const learnerName=String(displayName||'').trim().toLowerCase();const learnerCharacter=learnerName==='aeden'?{src:'/images/game/boy-idle.png',alt:'Aeden'}:learnerName==='lena'?{src:'/images/game/girl-idle.png',alt:'Lena'}:null;return <div className="screen today-screen home-v14"><section className="welcome"><div><p className="kicker">GOEDE DAG</p><h1>Salaam, {displayName}! <span>👋</span></h1><p>Vandaag is een mooie dag om te leren.</p></div><div className="streak-pill"><Flame/><b>{app.progress.streak||1}</b><span>dagen</span></div></section><div className="home-coach-stage" aria-hidden="true"><img className="home-coach-character" src={COACH_IMAGES.welcome} alt=""/></div><div className={`continue-card ${learnerCharacter?'has-learner-character':''}`} onClick={()=>go(game.game.lastTab&&game.game.lastTab!=='today'?game.game.lastTab:'path')}><div className="continue-copy"><small>GA VERDER WAAR JE WAS</small><h2>Verder leren</h2><div className="mini-progress"><i style={{width:`${Math.min(100,game.daily)}%`}}/></div></div>{learnerCharacter&&<div className="continue-learner-character" aria-hidden="true"><img src={learnerCharacter.src} alt="" draggable="false"/></div>}<button aria-label="Verder leren"><ChevronRight/></button></div><section className="quick-grid home-actions"><Quick title="Spelen" sub="Verdien vliegerpunten" icon={<Trophy/>} onClick={()=>go('games')}/><Quick title="Woorden" sub="Flashcards" icon={<Layers3/>} onClick={()=>go('words')}/><Quick title="Zinnen" sub="Oefen zinnen" icon={<MessageCircle/>} onClick={()=>go('sentences')}/><Quick title="Uitspraak" sub="Luister & spreek" icon={<Mic/>} onClick={()=>go('speak')}/></section><div className="review-banner home-review" onClick={()=>go('games')}><div className="seal"><Brain/></div><div><b>Nog oefenen</b><span>Korte challenge met wat extra aandacht nodig heeft.</span></div><ChevronRight/></div><section className="home-progress"><div className="home-progress-title"><span>JOUW VOORTGANG</span><b>Vandaag</b></div><GameSummary game={game}/><div className="profile-stats"><StatCard icon={<Layers3/>} value={known} label="Woorden"/><StatCard icon={<Trophy/>} value={game.xp} label="Vliegerpunten"/><StatCard icon={<Flame/>} value={app.progress.streak||1} label="Streak"/></div></section></div>}

function LearningPath({app,game,isAdmin=false,openLesson}){const completed=new Set(game.game.completedLevels||[]),current=Math.min(50,completed.size+1),adminOpen=isAdmin&&!!game.game.adminAllLevels;return <div className="screen curriculum-screen"><PageHead eyebrow="JOUW ROUTE" title="Van eerste woord tot gesprek" sub="50 levels. Elk level combineert plaatjes, luisteren, zinnen bouwen en een snelle ronde." badge={<><Sparkles/> {game.xp} VP</>}/><div className="curriculum-intro"><div><small>DOEL</small><b>Na level 50 kun je veel dagelijkse Afghaanse gesprekken begrijpen en zelf voeren.</b></div><span>{completed.size}/50</span></div>{adminOpen&&<div className="admin-unlock-banner"><ShieldCheck/> Admin testmodus: alle 50 levels zijn speelbaar.</div>}<div className="path-list curriculum-path">{CURRICULUM.map(l=>{const done=completed.has(l.number),locked=!adminOpen&&l.number>current,results=game.game.levelResults?.[l.number]||{},missions=new Set(MISSION_TYPES.filter(m=>results[m]?.passed===true));return <div className={`path-item curriculum-level ${done?'done':''} ${locked?'locked':''}`} key={l.id}><span className={`path-node ${done?'complete':''}`}>{done?<Check/>:locked?<Lock/>:l.number}</span><button className="lesson-row" disabled={locked} onClick={()=>openLesson(l)}><span className="lesson-art">{l.emoji}</span><div className="lesson-main"><div className="level-title-row"><b>{l.number}. {l.title}</b><em>{l.difficulty}</em></div><small>{l.words.length} woorden · {missions.size}/4 spellen boven 80%</small><div className="tiny-bar"><i style={{width:`${done?100:missions.size*25}%`}}/></div></div>{!locked&&<ChevronRight/>}</button>{done&&<div className="kite-earned"><Sparkles/> Vlieger vrijgespeeld</div>}</div>})}</div></div>}
...
function Games({app,game,go,isAdmin=false,selectedLesson,clearSelectedLesson}){
 ...
 const results=game.game.levelResults?.[level]||{},missions=new Set(MISSION_TYPES.filter(m=>results[m]?.passed===true)),adminOpen=isAdmin&&!!game.game.adminAllLevels,locked=!adminOpen&&level>game.level,allPassed=MISSION_TYPES.every(m=>results[m]?.passed===true);
 const choose=t=>{if(locked)return;if(t==='kite'){if(!allPassed||(game.game.kiteTickets||0)<=0)return;game.updateGame(g=>({...g,kiteTickets:Math.max(0,(g.kiteTickets||0)-1)}))}setType(t);game.updateGame(g=>({...g,lastGame:t}))};
 const backToMenu=()=>setType(null); const solved=kind=>{if(kind==='done')setType(null)}; const kiteComplete=n=>{if(n>0)awardXP(game,n*5,'vlieger avontuur',null)};
 if(type){return <div className={`screen focus-screen games-focus ${type==='kite'?'kite-game-active':''}`}><button className="focus-exit" onClick={backToMenu}><ChevronLeft/> Level {level}</button>{type!=='kite'&&<PageHead eyebrow={`LEVEL ${level} · ${lesson.difficulty}`} title={type==='sentence'?'Bouw de zin':type==='listen'?'Luisteren':type==='picture'?'Plaatjes':'Snelle ronde'} sub={`${lesson.title} · hulp en fouten verlagen je vliegerpunten.`}/>}<ChallengeErrorBoundary onReset={backToMenu}>{type==='sentence'?<SentenceBuilder game={game} level={level} lesson={lesson} onSolved={solved}/>:type==='listen'?<ListeningQuiz game={game} level={level} lesson={lesson} onSolved={solved}/>:type==='picture'?<PictureQuiz game={game} level={level} lesson={lesson} onSolved={solved}/>:type==='speed'?<SpeedRound game={game} level={level} lesson={lesson} onSolved={solved}/>:<KiteGameErrorBoundary onReset={backToMenu}><KiteAdventure onExit={backToMenu} onComplete={kiteComplete}/></KiteGameErrorBoundary>}</ChallengeErrorBoundary></div>}
 return <div className="screen games-home-safe level-hub"><button className="focus-exit" onClick={()=>{if(selectedLesson){clearSelectedLesson?.();go('path')}else go('today')}}><X/> Sluiten</button><PageHead eyebrow={`LEVEL ${level} VAN 50 · ${lesson.difficulty}`} title={lesson.title} sub={`${lesson.words.length} nieuwe woorden · oefenen tot je alle vier missies hebt voltooid.`} badge={<><Sparkles/> {game.xp} VP</>}/><div className="level-mission-strip"><div><small>LEVELVOORTGANG</small><b>{missions.size}/4 missies</b></div><div className="mission-dots">{MISSION_TYPES.map(m=><i key={m} className={missions.has(m)?'done':''}/>)}</div></div><div className="games-safe-grid"><button className={missions.has('picture')?'mission-done':''} onClick={()=>choose('picture')}><span><Layers3/></span><b>Plaatjes</b><small>Herken betekenis en beeld · max 10 VP</small>{missions.has('picture')&&<Check/>}</button><button className={missions.has('listen')?'mission-done':''} onClick={()=>choose('listen')}><span><Headphones/></span><b>Luisteren</b><small>Begrijp wat je hoort · max 15 VP</small>{missions.has('listen')&&<Check/>}</button><button className={missions.has('sentence')?'mission-done':''} onClick={()=>choose('sentence')}><span><BookText/></span><b>Bouw de zin</b><small>Zet taal actief in elkaar · max 20 VP</small>{missions.has('sentence')&&<Check/>}</button><button className={missions.has('speed')?'mission-done':''} onClick={()=>choose('speed')}><span><Zap/></span><b>Snelle ronde</b><small>Minimaal 3 goed voor de missie</small>{missions.has('speed')&&<Check/>}</button><button className={`kite-choice ${(!allPassed||(game.game.kiteTickets||0)<=0)?'kite-locked':''}`} disabled={!allPassed||(game.game.kiteTickets||0)<=0} onClick={()=>choose('kite')}><span>{(game.game.kiteTickets||0)>0?<Sparkles/>:<Lock/>}</span><b>Vlieger Avontuur</b><small>{allPassed&&(game.game.kiteTickets||0)>0?`${game.game.kiteTickets} verdiend · speel nu`:'Haal alle 4 leerspellen met meer dan 80% goed'}</small></button></div><div className="reward-rules"><b>Zo verdien je meer vliegerpunten</b><span>Zonder hulp = maximale score · hint = minder · fouten = steeds iets minder · antwoord van Farangis = halve basisbeloning. Het Vlieger Avontuur wordt pas vrijgespeeld als je alle vier leerspellen volledig afrondt met meer dan 80% goed.</span></div><GameSummary game={game}/></div>
}
...
function Words({app,game,go,selectedLesson,clearSelectedLesson}){const[query,setQuery]=useState(''),[idx,setIdx]=useState(game.game.positions?.words||0),[revealed,setRevealed]=useState(false),[dragX,setDragX]=useState(0),[dragging,setDragging]=useState(false),[entering,setEntering]=useState(false),start=useRef(null),[exit,setExit]=useState(null);const filtered=useMemo(()=>{const ids=selectedLesson?new Set(selectedLesson.words.map(x=>x.id)):null;return vocab.filter(v=>(!ids||ids.has(v.id))&&(!query||`${v.dutch} ${v.spoken} ${v.latin}`.toLowerCase().includes(query.toLowerCase())))},[query,selectedLesson]);const w=filtered[idx%Math.max(1,filtered.length)]||vocab[0],known=app.knownIds.has(w.id);const finish=right=>{if(exit||!filtered.length)return;app.update(p=>{const set=new Set(p.known||[]);right?set.add(w.id):set.delete(w.id);return{...p,known:[...set]}});practice(game,`word:${w.id}`,right);setExit(right?'right':'left');setDragX((right?1:-1)*Math.max(window.innerWidth*1.35,700));setTimeout(()=>{const n=(idx+1)%filtered.length;setIdx(n);remember(game,'words',n);setRevealed(false);setDragging(false);setDragX(0);setExit(null);setEntering(true);requestAnimationFrame(()=>requestAnimationFrame(()=>setEntering(false)))},300)};const cardStyle=entering?{transform:'translateY(110px) scale(.97)',opacity:.15,transition:'none'}:{transform:`translateX(${dragX}px) rotate(${dragX/24}deg)`,opacity:exit?0.2:1,transition:dragging?'none':'transform .30s ease, opacity .28s ease'};return <div className="screen focus-screen words-focus"><button className="focus-exit" onClick={()=>go('today')}><X/> Sluiten</button><PageHead eyebrow="1000+ WOORDEN" title="Woorden" sub="Swipe rechts: Ken ik. Links: Nog oefenen. Hier verdien je geen VP." badge={<>{idx+1} / {filtered.length}</>}/>{selectedLesson&&<div className="review-banner"><div className="seal">{selectedLesson.emoji}</div><div><b>Les {selectedLesson.number} · {selectedLesson.title}</b><span>{selectedLesson.words.length} woorden</span></div><button onClick={clearSelectedLesson}><X/></button></div>}<div className="word-toolbar"><label><Search/><input value={query} onChange={e=>{setQuery(e.target.value);setIdx(0)}} placeholder="Zoek Nederlands of fonetisch…"/></label></div><div className={`swipe-stage ${exit?'card-is-exiting':''} ${entering?'card-is-entering':''}`}><article key={`${w.id}-${idx}`} className="premium-flashcard swipe-card swipe-card-top" style={cardStyle} onPointerDown={e=>{if(exit||entering)return;start.current=e.clientX;setDragging(true)}} onPointerMove={e=>{if(start.current!==null&&!exit)setDragX(Math.max(-240,Math.min(240,e.clientX-start.current)))}} onPointerUp={()=>{setDragging(false);start.current=null;if(Math.abs(dragX)>85)finish(dragX>0);else setDragX(0)}} onClick={()=>Math.abs(dragX)<8&&!exit&&setRevealed(r=>!r)}><div className={`swipe-stamp learn ${dragX<-25?'show':''}`}>NOG OEFENEN</div><div className={`swipe-stamp know ${dragX>25?'show':''}`}>KEN IK!</div><div className="card-top"><span>{labelFor(w.category)}</span></div><WordIllustration word={w}/><div className="word-copy"><small>NEDERLANDS</small><h2>{w.dutch}</h2>{revealed?<><small>ZO ZEG JE HET</small><h3>{w.spoken||w.latin}</h3><button className="sound-btn" onClick={e=>{e.stopPropagation();speak(w.spoken||w.latin,.88,'word',w.id)}}><Volume2/> Luister</button></>:<div className="reveal-hint"><Eye/> Tik voor vertaling</div>}</div></article></div><div className="card-nav swipe-actions-row"><button onClick={()=>finish(false)}><X/> Nog oefenen</button><button onClick={()=>finish(true)}><Check/> Ken ik!</button></div></div>}
