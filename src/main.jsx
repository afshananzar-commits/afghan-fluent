...
function Sentences({app}){
 const [query,setQuery]=useState(''); const [playing,setPlaying]=useState(null);
 const data=useMemo(()=>sentences.filter(s=>(!query||`${s.dutch} ${s.spoken||''} ${s.latin||''}`.toLowerCase().includes(query.toLowerCase()))),[query]);
