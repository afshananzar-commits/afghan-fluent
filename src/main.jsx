...
function SentenceBuilder({game,level,lesson,onSolved}){
 ...
 const check=()=>{if(answer.some(x=>!x))return;const ok=answer.every((t,i)=>t?.origin===i);sessionStats.current.total+=1;if(ok)sessionStats.current.correct+=1;setResult(ok?'good':'bad');practice(game,`sentence:${s.id}`,ok);if(ok){const pts=rewardFor(20,help);awardXP(game,pts,'zin gebouwd',level);setCoachMessage({type:'correct',text:`Perfect! +${pts} 🪁`})}else{setHelp(h=>({...h,wrong:h.wrong+1}));setCoachMessage({type:'almost',text:'Bijna — probeer het nog eens. Bij een volgende poging zijn iets minder punten te verdienen.'})}};
 ...
 return <section className={`game-card sentence-builder-v2 sentence-builder-v29 ${answer.length>3?'has-many-words':'has-three-words'}`} style={{'--word-count':Math.max(1,answer.length),'--sentence-tile-font':`${tileFontSize}px`}}>
   <div className="builder-v29-head">
     <div className="builder-v29-title">
       <small>🧩 BOUW DE ZIN <span className="builder-xp-inline">max 20 🪁</span></small>
...
function Words({app,game,go,selectedLesson,clearSelectedLesson}){
 const[query,setQuery]=useState(''),[idx,setIdx]=useState(game.game.positions?.words||0),[revealed,setRevealed]=useState(false),[dragX,setDragX]=useState(0),[dragging,setDragging]=useState(false),start=useRef(null),[exit,setExit]=useState(null);
 const filtered=useMemo(()=>{const ids=selectedLesson?new Set(selectedLesson.words.map(x=>x.id)):null;return vocab.filter(v=>(!ids||ids.has(v.id))&&(!query||`${v.dutch} ${v.spoken} ${v.latin}`.toLowerCase().includes(query.toLowerCase())))},[query,selectedLesson]);
 const w=filtered[idx%Math.max(1,filtered.length)]||vocab[0],nextW=filtered[(idx+1)%Math.max(1,filtered.length)]||w;
 const finish=right=>{if(exit||!filtered.length)return;app.update(p=>{const set=new Set(p.known||[]);right?set.add(w.id):set.delete(w.id);return{...p,known:[...set]}});practice(game,`word:${w.id}`,right);setExit(right?'right':'left');setDragX((right?1:-1)*Math.max(window.innerWidth*1.35,700));setTimeout(()=>{const n=(idx+1)%filtered.length;setIdx(n);remember(game,'words',n);setRevealed(false);setDragging(false);setDragX(0);setExit(null)},300)};
