...
function AdminPanel({session,profile,enabledMissions=MISSION_TYPES,onMissionToggle,missionConfigStatus=''}){
 const[users,setUsers]=useState([]),[name,setName]=useState(''),[password,setPassword]=useState(''),[busy,setBusy]=useState(false),[msg,setMsg]=useState('');
 ...
 return <div className="admin-screen"><PageHead eyebrow="ADMIN" title="Beheer" sub="Gebruikers, voortgang en de stem van Farangis."/>
  ...
  <section className="admin-card"><h3>Gebruikers</h3><div className="admin-users">{users.map(u=><div className="admin-user" key={u.id}><div className="admin-avatar">{(u.display_name||'?')[0]}</div><div><b>{u.display_name}</b><span>{u.role==='admin'?'Administrator':'Leerling'}</span></div>{u.role!=='admin'&&<button onClick={()=>resetUser(u.id)}><RefreshCw/> Reset</button>}</div>)}</div>
   <form className="admin-create" onSubmit={createUser}><h4>Nieuw account</h4><input placeholder="Naam, bijvoorbeeld Aeden" value={name} onChange={e=>setName(e.target.value)} required/><input placeholder="Tijdelijk wachtwoord" type="password" value={password} onChange={e=>setPassword(e.target.value)} minLength="6" required/><button disabled={busy}><UserRound/> Account aanmaken</button></form>
  </section>
...
function Today({app,game,go,displayName='Leerling'}){
  ...
  return <div className="screen today-screen home-v14 home-v16">
    <section className="welcome home-welcome-v2"><div className="welcome-copy"><p className="kicker">GOEDE DAG</p><h1>Salaam, {displayName}! <span>👋</span></h1><p className="welcome-line">Vandaag is een mooie dag om te leren.</p></div><div className="home-coach-stage" aria-hidden="true"><img className="home-coach-character" src={COACH_IMAGES.welcome} alt=""/></div></section>
...
function App(){
 ...
 const[tab,setTab]=useState('today'),[selectedLesson,setSelectedLesson]=useState(null),[enabledMissions,setEnabledMissions]=useState(()=>readMissionConfig()),[missionConfigError,setMissionConfigError]=useState('');
 ...
 useEffect(()=>{
  ...
  loadCloudState(session).then(s=>{if(cancelled)return;setProfile(s.profile);setCloudProgress(s.progress||{});setCloudGame(s.game||{});latestCloudState.current={progress:s.progress||{},game:s.game||{}};cloudStateLoaded.current=true;
   ...
   setProfile({id:session.user.id,display_name:session.user.user_metadata?.display_name||session.user.email?.split('@')[0]||'Leerling',role:fallbackRole,is_active:true,_loadError:error.message});
  });
 ...
 return <div className="app"><DesktopRail tab={tab} go={go} streak={app.progress.streak||0} isAdmin={profile.role==='admin'}/><div className="app-stage"><TopChrome/><main>
  ...
  {tab==='profile'&&<Profile app={app} game={game} contentStatus={contentStatus} refreshContent={refreshContent} profile={profile} go={go} onLogout={()=>supabase.auth.signOut()}/>}
...
function Games({app,game,go,isAdmin=false,selectedLesson,clearSelectedLesson}){
 ...
 const tile=(m,Icon,title,accent,reward)=>{if(!enabled.includes(m))return null;const r=results[m],done=missions.has(m),blocked=!adminOpen&&!wordsDone;const chip={picture:{bg:'#DCEBDC',ink:'#225A40'},listen:{bg:'#F4D9D6',ink:'#A64941'},sentence:{bg:'#F1E2CD',ink:'#A86B20'},speed:{bg:'#F4DFA6',ink:'#A76812'}}[m];return <button key={m} className={`game-tile-v45 game-tile-v46 ${accent} ${done?'mission-done':''}`} disabled={blocked} onClick={()=>choose(m)}><span className="game-icon-chip game-icon-chip-v56" data-mission={m} style={{'--chip-bg':chip.bg,'--chip-ink':chip.ink}}>{blocked?<Lock/>:<Icon/>}</span><div className="game-tile-copy"><b>{title}</b><em>{blocked?'Eerst woorden leren':r?.total?`${r.accuracy}% goed`:reward}</em></div>{done&&<Check className="game-done-check"/>}</button>};
 return <div className="screen games-home-safe level-hub level-hub-v45 level-hub-v46 level-hub-v52 level-hub-v54 level-hub-v55">
