...
function loadProgress(){return readJsonStorage('afghanFluentProgress',{})} function saveProgress(p){localStorage.setItem('afghanFluentProgress',JSON.stringify(p))}
function useAppState(){const[progress,setProgress]=useState(loadProgress);const update=fn=>setProgress(p=>{const n=typeof fn==='function'?fn(p):fn;saveProgress(n);return n});return{progress,update,knownIds:new Set(progress.known||[]),favorites:new Set(progress.favorites||[])}}
function loadGame(){return readJsonStorage(GAME_KEY,{})} function saveGame(g){localStorage.setItem(GAME_KEY,JSON.stringify(g))}
function useGameState(){const[game,setGame]=useState(loadGame);const updateGame=fn=>setGame(g=>{const n=typeof fn==='function'?fn(g):fn;saveGame(n);return n});const xp=game.xp||0;return{game,updateGame,xp,level:Math.floor(xp/500)+1,levelXp:xp%500,daily:game.daily?.date===dayKey()?(game.daily.xp||0):0,goal:game.dailyGoal||100}}
...
function speak(text,rate=.88){if(!text||!('speechSynthesis'in window))return;window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);u.rate=rate;window.speechSynthesis.speak(u)}
function shuffle(a){a=[...a];for(let i=a.length-1;i;i--){const j=Math.floor(Math.random()*(i+1));[a[i],a[j]]=[a[j],a[i]]}return a}

function App(){
 const[tab,setTab]=useState('today'),[mode,setMode]=useState(localStorage.getItem('afghanFluentMode')||'family'),[selectedLesson,setSelectedLesson]=useState(null);
 ...
 const app=useAppState(),game=useGameState();
 ...
 useEffect(()=>{const t=dayKey();if(app.progress.lastOpen!==t){const y=new Date();y.setDate(y.getDate()-1);app.update(p=>({...p,lastOpen:t,streak:p.lastOpen===dayKey(y)?Math.max(1,(p.streak||0)+1):1}))}},[]);
 const go=n=>{setTab(n);game.updateGame(g=>({...g,lastTab:n}));window.scrollTo({top:0,behavior:'smooth'})};
 return <div className={`app ${mode==='kids'?'kids-mode':''}`}><DesktopRail tab={tab} go={go} streak={app.progress.streak||0}/><div className="app-stage"><TopChrome mode={mode} setMode={setModePersist}/><main>
  {tab==='today'&&<Today app={app} game={game} go={go}/>}
  ...
  {tab==='profile'&&<Profile app={app} game={game} mode={mode} setMode={setModePersist} contentStatus={contentStatus} refreshContent={refreshContent}/>}
 ...
 return <section className={`game-card sentence-builder-v2 sentence-builder-v29 ${answer.length>3?'has-many-words':'has-three-words'}`} style={{'--word-count':Math.max(1,answer.length)}}>
   <div className="builder-v29-head">
     ...
     <div className="builder-v29-tools" aria-label="Hulpmiddelen">
       ...
       <button onClick={()=>{speak(s.spoken||s.latin,.78);setCoachMessage({type:'listen',text:'Luister goed naar het ritme en zeg de zin daarna rustig na.'})}} aria-label="Luister" title="Luister">🎧</button>
