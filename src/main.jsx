...
function Games({app,game,go,isAdmin=false,selectedLesson,clearSelectedLesson}){
 ...
 return <div className="screen games-home-safe level-hub level-hub-v45">
  <button className="focus-exit" onClick={()=>{if(selectedLesson){clearSelectedLesson?.();go('path')}else go('today')}}><X/> Sluiten</button>
  <PageHead eyebrow={`LEVEL ${level} VAN 50 · ${lesson.difficulty}`} title={lesson.title} sub={`${lesson.words.length} nieuwe woorden · ${parts} van 5 onderdelen voltooid`} badge={<><img className="vp-kite-icon" src="/images/game/kite.png" alt=""/>{game.xp}</>}/>
  <div className="level-mission-strip level-mission-strip-v45">
    <div><small>JOUW VOORTGANG</small><b>{parts}/5 voltooid</b></div>
    <div className="mission-dots mission-dots-five"><i className={wordsDone?'done':''}/>{MISSION_TYPES.map(m=><i key={m} className={missions.has(m)?'done':''}/>)}</div>
  </div>
  <button className={`word-intro-choice word-intro-v45 ${wordsDone?'mission-done':''}`} onClick={()=>choose('words')}>
    <span><BookOpen/></span><div><b>{lesson.words.length} nieuwe woorden</b><small>{wordsDone?'Alle woorden bekeken · herhalen mag altijd':`${game.game.levelWordSeen?.[level]?.length||0}/${lesson.words.length} bekeken · leer deze eerst`}</small></div>{wordsDone&&<Check/>}
  </button>
  <div className="game-picker-label"><span>KIES EEN SPEL</span><small>Vier korte rondes om deze woorden echt te beheersen</small></div>
  <div className="games-safe-grid games-safe-grid-five games-grid-v45">
    <button className={`game-tile-v45 game-tile-picture ${missions.has('picture')?'mission-done':''}`} disabled={!adminOpen&&!wordsDone} onClick={()=>choose('picture')}><span>{!adminOpen&&!wordsDone?<Lock/>:<Layers3/>}</span><b>Plaatjes</b><small>{!adminOpen&&!wordsDone?'Eerst woorden leren':'Herken het juiste beeld'}</small><em>{missions.has('picture')?'Voltooid':'tot 10 🪁'}</em>{missions.has('picture')&&<Check/>}</button>
    <button className={`game-tile-v45 game-tile-listen ${missions.has('listen')?'mission-done':''}`} disabled={!adminOpen&&!wordsDone} onClick={()=>choose('listen')}><span>{!adminOpen&&!wordsDone?<Lock/>:<Headphones/>}</span><b>Luisteren</b><small>{!adminOpen&&!wordsDone?'Eerst woorden leren':'Kies wat je hoort'}</small><em>{missions.has('listen')?'Voltooid':'tot 15 🪁'}</em>{missions.has('listen')&&<Check/>}</button>
    <button className={`game-tile-v45 game-tile-sentence ${missions.has('sentence')?'mission-done':''}`} disabled={!adminOpen&&!wordsDone} onClick={()=>choose('sentence')}><span>{!adminOpen&&!wordsDone?<Lock/>:<BookText/>}</span><b>Bouw de zin</b><small>{!adminOpen&&!wordsDone?'Eerst woorden leren':'Zet woorden op volgorde'}</small><em>{missions.has('sentence')?'Voltooid':'tot 20 🪁'}</em>{missions.has('sentence')&&<Check/>}</button>
    <button className={`game-tile-v45 game-tile-speed ${missions.has('speed')?'mission-done':''}`} disabled={!adminOpen&&!wordsDone} onClick={()=>choose('speed')}><span>{!adminOpen&&!wordsDone?<Lock/>:<Zap/>}</span><b>Snelle ronde</b><small>{!adminOpen&&!wordsDone?'Eerst woorden leren':'Test wat je kent'}</small><em>{missions.has('speed')?'Voltooid':'snelle test'}</em>{missions.has('speed')&&<Check/>}</button>
  </div>
  <button className={`kite-choice kite-choice-v45 ${adminOpen?'kite-admin-open':((!allPassed||(game.game.kiteTickets||0)<=0)?'kite-locked':'')}`} disabled={!adminOpen&&(!allPassed||(game.game.kiteTickets||0)<=0)} onClick={()=>choose('kite')}><span>{adminOpen?<ShieldCheck/>:(game.game.kiteTickets||0)>0&&allPassed?<Sparkles/>:<Lock/>}</span><div><b>Vlieger Avontuur</b><small>{adminOpen?'Admin testmodus · altijd speelbaar':allPassed&&(game.game.kiteTickets||0)>0?`${game.game.kiteTickets} verdiend · speel nu`:'Voltooi alle spellen met 80%+ om vrij te spelen'}</small></div></button>
 </div>
}
